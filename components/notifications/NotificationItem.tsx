...
export default function NotificationItem({
  ...
  return (
    <button
      onClick={onClick}
      className="flex items-start gap-3 w-full text-left px-4 py-3 hover:bg-[var(--bg-elevated)] transition-all rounded-lg"
    >
      {!notification.read && (
      ...
      <div className="min-w-0 flex-1">
        <div className={`text-sm font-medium ${notification.read ? "text-[var(--text-secondary)]" : "text-[var(--text-primary)]"}`}>
          {notification.title}
        </div>
